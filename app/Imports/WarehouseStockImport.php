...
class WarehouseStockImport implements ToModel, WithHeadingRow
{
    /**
     * @param array $row
    ...
    public function model(array $row)
    {
        ...
        // Find existing product by product code or other unique identifier
        $existingProduct = Product::where('product_code', $row['product_code'])->first();

        if ($existingProduct) {
            // Update only the product_store value
            $existingProduct->product_store += (int) $row['product_store'];
